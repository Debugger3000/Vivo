...
class PatchEventForm extends StatefulWidget {
  final GetEventPreview event; // receives event from navigator call

  const PatchEventForm({super.key, required this.event});

  @override
  State<PatchEventForm> createState() => PatchEventFormState();
}

class PatchEventFormState extends State<PatchEventForm> {
  ...
  double curLat = 44.389355; // current Lat
  double curLng = -79.690331; // current lng
  String curAddress = "18 Claudio Crescent"; // cur address
...
// ------------------------------------------------------------------------------
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      print("set state for patch event page");
      grabbedCategories = categories;
      _markerSet = {MarkerBuilder.build(lat: widget.event.latitude, lng: widget.event.longitude, title: widget.event.title, description: widget.event.description)};
  ...
  void setFields() {
    ...
    curLat = widget.event.latitude;
    curLng = widget.event.longitude;

    // tags / categories
    selectedTags = widget.event.tags;
...
// SUBMIT EVENT
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: SubPageHeader(title: 'Edit Event'),
      ...
      child:  Form(
        ...
        child: Padding(
          ...
          children: [
            ...
            PlacesSearch(onPlaceSelectedCallback: _goToAddress, address: widget.event.address),
